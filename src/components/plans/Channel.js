...
import {
  isChannelOptionDisabled,
  getChannelLabel,
} from "../../../src/app/(site)/plans/utils/channelUtils";
            ...
            <div>
              <div className="plans-tabs">
                {tvChannel.map((option) => {
                  const disabled = isChannelOptionDisabled(
                    activePricing,
                    option
                  );
                  return (
                    <div
                      key={option}
                      className={`plan-tab ${
                        activeChannel === option ? "plan-active" : ""
                      } ${disabled ? "disabled" : ""}`}
                      onClick={() => {
                        if (!disabled) {
                    ...
                    >
                      {option}
                      <span className="include-wrap">
                        {getChannelLabel(activePricing, option)}
                      </span>
                    </div>
                  );
                })}
