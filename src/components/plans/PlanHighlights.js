  ...
  const calculatePlanPrice = useMemo(() => {
    ...
    const installationCharge =
      ...
      ["Monthly", "Quarterly", "Half Yearly", "Yearly"].includes(
        ...
        ? 1500
        : activeTab === "50 Mbps" &&
          ["Monthly", "Quarterly"].includes(activeNestedTab)
        ? 1000
        : [
            "100 Mbps",
            "200 Mbps",
            ...
            "500 Mbps",
            "1000 Mbps",
          ].includes(activeTab) && activeNestedTab === "Monthly"
        ? 1000
        : 0;
    const channelPrice = calculateAddonPrice(
      activeChannel,
      activeNestedTab === "Monthly"
        ? 1
        : activeNestedTab === "Quarterly"
        ? 3
        : activeNestedTab === "Half Yearly"
        ? 6
        : activeNestedTab === "Yearly"
        ? 12
        : 0
    );
    const ottPrice = calculateAddonPrice(
      activeOtts,
      activeNestedTab === "Monthly"
        ? 1
        : activeNestedTab === "Quarterly"
    ...
    let subtotal =
      Number(basePrice) + installationCharge + channelPrice + ottPrice;
              ...
              <div className="featuresWrap">
                <div className="table-channel">
                  <div className="tv-channels">
                    <img
                      alt="speed-icon"
                      src="https://www.skylink.net.in/wp-content/uploads/2025/04/speed-icon.svg"
                    />
                  </div>
                </div>
                <div className="content-wrap">
                  <div className="featuresDescription">{activeTab}</div>
                  <div className="featuresTitle">Speed</div>
                </div>
              </div>
              {pricing[activeTab]?.[activeNestedTab]?.benefits.map(
                (benefit, index) => (
                  <div
                    className="featuresWrap"
                    key={`${activeTab}-benefit-${index}`}
                  >
                    <div className="table-channel">
                      {benefit.name === "TV Channels" && (
                        <div className="tv-channels tv-wrap">
                          <CurrentChannel
                            benefit={benefit}
                      ...
                      {benefit.name.startsWith("24/7 Elite Support") && (
                        <div className="tv-channels">
                          ...
                          <img
                            alt="elite-support"
                            src="https://www.skylink.net.in/wp-content/uploads/2025/04/elite-support.svg"
                          />
                        </div>
                      )}
                    ...
                    <div className="content-wrap">
                      <div className="featuresDescription">
                        <div className="" key={`${activeTab}-benefit-${index}`}>
                          {benefit.lite}
                        </div>
                      </div>
      ...
      <tr style={{ display: "none" }}>
        <td className="padding-0 highlights-page-image">
          <img
            width="100%"
            style={{ width: "100%" }}
            src="https://skylink.net.in/wp-content/uploads/2025/03/dummy-banner.png"
            alt="Plan Banner"
          />
        </td>
        <td className="pricing-features-wrap" colSpan={colSpan}>
          <div className="pricing-features">
            <span className="price">
              ...
              <span className="gst-text">* Exclude GST 18%</span>
              <span
                style={{
                  fontSize: "14px",
                  color: "black",
          ...
          <button
            className="subscribe new"
            data-price={calculatePlanPrice}
