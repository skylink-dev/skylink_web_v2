...
export default function CustomPlans() {
  ...
  const billingAddonCycleMultiplier =
    billingAddonMultipliers[activeNestedTab] || 1;
  ...
  const getInstallationFee = (speed, billingCycle) => {
    ...
    if (speed === "50 Mbps" && ["Monthly", "Quarterly"].includes(billingCycle))
      return 1000;
    if (
      [
        "100 Mbps",
  ...
  const getOttPrice = (activeott) => {
    const basePrice =
      pricing[activeTab]?.[activeNestedTab]?.ott === activeott
        ? 0
        : ottPrices[activeott];
  ...
  const getChannelPrice = (activeChannel) => {
    const basePrice =
      pricing[activeTab]?.[activeNestedTab]?.tv === activeChannel
        ? 0
        : channelPrices[activeChannel];
    if (basePrice === 0) return 0;
  ...
  const channelPrice =
    (getChannelPrice(activeChannel) || 0) * billingAddonCycleMultiplier;

  const ottPrice = (getOttPrice(activeOtts) || 0) * billingAddonCycleMultiplier;

  const totalPrice =
    Number(basePrice) + installationCost + channelPrice + ottPrice;
      ...
      <div className="custom-plan-inner-wrap">
        <div className="firstset">
          ...
          <div style={{ display: "flex" }}>
            <div
              className={`plan-details-features transition-all duration-500 ease-in-out ${
                isOpen ? "show" : "hide"
              }`}
            >
              <span className="plan-details-feature-list">
                <span className="feature-list-head">Speed</span>{" "}
                <span className="feature-list-ans">{activeTab}</span>
              </span>
              <span className="plan-details-feature-list">
                <span className="feature-list-head">Billing Cycle</span>{" "}
                <span className="feature-list-ans">
                  ₹{staticBasePrice} * {billingCycleMultiplier}
                </span>
              </span>
              <span className="plan-details-feature-list">
                <span className="feature-list-head">Installation</span>{" "}
                <span className="feature-list-ans">₹{installationCost}</span>
              </span>
              <div className="addon-price-wrap">
                <h3>
                  <div className="tv-channel-title">
                    {activeChannel ||
                      pricing?.[activeTab]?.[activeNestedTab]?.tv}{" "}
                    <span className="hd-mention-text">
                      {activeChannel === "650+ Channels"
                        ? "(94+ HD)"
                        : activeChannel === "950+ Channels"
                        ? "(ALL HD)"
                        : ""}
                    </span>
                  </div>
                        ...
                        return label === 0 ? (
                          ...
                          <>
                            <i
                              ...
                              style={{ fontSize: "10px" }}
                            ></i>{" "}
                            {label} × {billingAddonCycleMultiplier}
                          </>
                        );
                      })()}
                    </span>
                ...
                <div className="tv-channels" style={{ display: "none" }}>
                  <CurrentChannel
              ...
              <div className="addon-price-wrap">
                        ...
                        return price === 0 ? (
                          ...
                          <>
                            <i
                              ...
                              className="fas fa-rupee-sign"
                            ></i>{" "}
                            {price} × {billingAddonCycleMultiplier}
                          </>
                        );
                      })()}
                    </span>
                ...
                <div style={{ display: "none" }}>
                  <CurrentOTT
                    activeOtts={String(
                      pricing[activeTab]?.[activeNestedTab]?.ott
                    )?.trim()}
                    activeTab={activeTab}
                    activeNestedTab={activeNestedTab}
                  />
                </div>
              </div>
            </div>
        ...
        <div className="pricing-features">
          ...
          >
            <span className="plan-details-feature-list">
              ...
              <span className="feature-list-ans">₹{getOriginalPrice()}</span>
            </span>
            <span className="plan-details-feature-list">
              <span className="feature-list-head">Discounts</span>{" "}
              <span className="feature-list-ans">- ₹{getDiscountAmount()}</span>
            </span>
            <span className="plan-details-feature-list">
            ...
            <span className="plan-details-feature-list">
              ...
              <span className="feature-list-ans">+ ₹{gstAmount}</span>
            </span>
          </div>
        </div>
        <div className="destop-subscribe-style">
          ...
          <div className="subscribe-wrap">
            <button
              className="subscribe"
              data-price={getFinalPriceWithGst()}
