...
const plansSlice = createSlice({
  ...
  initialState: {
    ...
    activePrice: 699,
    activePlanPricing:
      pricingByZone["Tamil Nadu"]["Fixed Plan"]["50mbps"]["699"]["Monthly"],
  },
  reducers: {
    setActiveZoneIndex(state, action) {
      ...
      if (newIndex >= 0 && newIndex < state.planOptions.zones.length) {
        ...
        const defaultPlanId = Object.keys(
          state.planOptions.actualPrizeByZone[zoneName]?.[defaultBundle]?.[
            defaultMbps
          ] || {}
        )[0];
        ...
        state.activePlanPricing =
          state.planOptions.actualPrizeByZone[zoneName]?.[defaultBundle]?.[
            defaultMbps
          ]?.[defaultPlanId]?.[state.activeBillingCycle] || null;
      }
    },
    ...
    setActiveBundleIndex(state, action) {
      ...
      const mbpsList = Object.keys(
        state.planOptions.actualPrizeByZone[zoneName]?.[bundle] || {}
      );
      const firstMbps = mbpsList[0];
      const planKeys = Object.keys(
        state.planOptions.actualPrizeByZone[zoneName]?.[bundle]?.[firstMbps] ||
          {}
      );
    ...
    setActiveMbpsIndex(state, action) {
      ...
      const zoneName = state.planOptions.zones[state.activeZoneIndex];
      const planGroup =
        state.planOptions.actualPrizeByZone[zoneName]?.[state.activeBundle]?.[
          newMbps
        ];
      const firstPlanKey = Object.keys(planGroup || {})[0];
      state.activePlanPricing =
        planGroup?.[firstPlanKey]?.[state.activeBillingCycle] || null;
      state.activePlanNameIndex = 0;
    },
    setActivePlanNameIndex(state, action) {
      const newIndex = action.payload;
      const zoneName = state.planOptions.zones[state.activeZoneIndex];
      const planList = Object.keys(
        state.planOptions.actualPrizeByZone[zoneName]?.[state.activeBundle]?.[
          state.activeMbpsIndex
        ] || {}
      );
      ...
      state.activePlanPricing =
        state.planOptions.actualPrizeByZone[zoneName]?.[state.activeBundle]?.[
          state.activeMbpsIndex
        ]?.[selectedPlan]?.[state.activeBillingCycle] || null;
    },
